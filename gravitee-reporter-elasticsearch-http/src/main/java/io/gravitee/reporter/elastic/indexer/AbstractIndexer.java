...
import io.gravitee.reporter.elastic.indexer.name.IndexNameGenerator;
...
public abstract class AbstractIndexer implements Indexer {
    ...
    protected Single<String> transform(Reportable reportable) {
        return Single
                .just(reportable)
                .flatMap(new Function<Reportable, SingleSource<String>>() {
                    @Override
                    public SingleSource<String> apply(Reportable reportable) throws Exception {
                        if (reportable instanceof Metrics) {
                            return Single.just(getSource((Metrics) reportable, pipelineConfiguration.getPipeline()));
                        } else if (reportable instanceof EndpointStatus) {
                            return Single.just(getSource((EndpointStatus) reportable));
                        } else if (reportable instanceof Monitor) {
                            return Single.just(getSource((Monitor) reportable));
                        } else if (reportable instanceof Log) {
                            return Single.just(getSource((Log) reportable));
                        }
    ...
    protected String getSource(final Metrics metrics, String pipeline) {
        ...
        data.put("type", Types.TYPE_REQUEST);
    ...
    protected String getSource(final EndpointStatus endpointStatus) {
        ...
        data.put("type", Types.TYPE_HEALTH);
    ...
    protected String getSource(final Monitor monitor) {
        ...
        data.put("type", Types.TYPE_MONITOR);
